...
import { decreaseDangChon, increaseDangChon } from '../../redux/VephimSlice';
...
const Drops = () => {
  ...
  const tongTien = () => {
    const giaGhe = 75000;
    return selectedSeats.length * giaGhe;
  };

  const dispatch = useDispatch();

  const dangchon = useSelector((state) => state.vephim.dangchon);
  const chuachon = useSelector((state) => state.vephim.chuachon);

  const [selectedSeats, setSelectedSeats] = useState([]);

  const handleChoNgoi = (chongoi) => {
    setSelectedSeats((prevSelectedSeats) => {
      if (prevSelectedSeats.some(s => s.soGhe === chongoi.soGhe)) {
        dispatch(decreaseDangChon());
        return prevSelectedSeats.filter((s) => s.soGhe !== chongoi.soGhe);
      } else {
        dispatch(increaseDangChon());
        return [...prevSelectedSeats, chongoi];
      }
    });
  };
  ...
  return (
    <div className="bg z-20">
      <div className='container flex flex-row space-x-28 pt-9'>
        <div>
          <h2 className='text-3xl text-center text-yellow-500 bookingMovie'>
            Đặt vé xem phim tại CYBERLEARN.VN
          </h2>
          <h3 className='m-8 rounded-full bg-orange-500 p-5 text-3xl font-normal text-center bookingMovie'>Màn Hình</h3>
          <div>
            <Shose arrChongoi={arrChongoi} handleChoNgoi={handleChoNgoi} selectedSeats={selectedSeats} />
          </div>
        </div>
        <div className='space-y-10 mt-20 bookingMovie relative'>
          ...
          <div className='square bg-white'>
            <span className='absolute left-16 text-2xl'>Ghế chưa chọn: {chuachon}</span>
          </div>
          <div>
            <span className='text-3xl text-red-500'>Tổng tiền: {tongTien().toLocaleString()} VND</span>
          </div>
        </div>
      </div>
    </div>
